...
export default function FixedCustomDeductionsSettings() {
  ...
  // Temporary Deductions Form State
  ...
  const handleUpdateTemporary = async (e) => {
    ...
    try {
      ...
      toast({
        title: 'تم تحديث الاستقطاع المؤقت بنجاح',
        variant: 'success',
      });
      ...
      await apiClient.logs.create({
        action: 'تعديل استقطاع مؤقت',
        details: `تم تعديل الاستقطاع المؤقت "${tempName}" ${dateDetails}`,
        userId: user?.id
      });
    } catch (error) {
      console.error('Error updating temporary deduction:', error);
      toast({
  ...
  const handleAddTemporary = async (e) => {
    ...
    try {
      ...
      toast({
        title: 'تم حفظ الاستقطاع المؤقت بنجاح',
        variant: 'success',
      });
      ...
      const dateDetails = tempTimingType === 'range' 
        ...
        : `لشهر ${tempPaymentMonth}/${tempPaymentYear}`;
      await apiClient.logs.create({
        action: 'إضافة استقطاع مؤقت جديد',
        details: `تمت إضافة استقطاع مؤقت باسم "${tempName}" ${dateDetails}`,
        userId: user?.id
      });
    } catch (error) {
      console.error(error);
      toast({
        title: 'فشل حفظ الاستقطاع المؤقت',
        variant: 'destructive',
      });
    }
  };
  
  // 1. Fixed Deductions State
  const [retirementRate, setRetirementRate] = useState(5); // as percentage (5%)
  const [retirementStatus, setRetirementStatus] = useState('فعال');
  const [savingFixed, setSavingFixed] = useState(false);

  // 2. Custom Deductions State
  const [records, setRecords] = useState([]);
  const [loading, setLoading] = useState(false);
  const [adding, setAdding] = useState(false);
  
  // Form state for adding custom deduction
  ...
  const handleToggleStatus = async (record) => {
    try {
      ...
      toast({
        ...
        description: `تم ${newStatus === 'فعال' ? 'تفعيل' : 'إيقاف'} بند الاستقطاع "${record.name}" لجميع الموظفين.`,
        variant: 'success',
      });
      ...
      await apiClient.logs.create({
        ...
        details: `تغيير حالة بند الاستقطاع (${record.name}) إلى: ${newStatus}`
      }).catch(() => {});
    } catch (error) {
      toast({
  ...
  const fetchCustomRecords = async () => {
    ...
    try {
      ...
      const savedOrder = localStorage.getItem('CUSTOM_DEDUCTIONS_ORDER');
      if (savedOrder) {
        try {
          ...
          console.error('Error parsing CUSTOM_DEDUCTIONS_ORDER:', e);
        }
      }
  ...
  const handleDrop = (e, targetIndex) => {
    ...
    localStorage.setItem('CUSTOM_DEDUCTIONS_ORDER', JSON.stringify(nextRecords.map(r => r.id)));
    ...
    toast({
      ...
      description: 'تم تحديث ترتيب الاستقطاعات المخصصة بنجاح',
    });
  };
  ...
  const handleAddCustom = async (e) => {
    ...
    if (!newName.trim() || !newValue) {
      toast({
        ...
        description: 'يرجى إدخال اسم الاستقطاع وقيمته',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: 'تمت إضافة الاستقطاع المخصص',
        description: `تمت إضافة بند استقطاع مخصص باسم "${newName}" بنجاح`,
        variant: 'success',
      });
      ...
      // Log action
      await apiClient.logs.create({
        action: 'إضافة استقطاع مخصص جديد',
        details: `إضافة بند مالي استقطاع جديد (${newName} - طريقة الاحتساب: ${newCalcType === 'percentage' ? 'نسبة مئوية' : 'مبلغ مقطوع'}، القيمة: ${newValue})`
      }).catch(() => {});
    } catch (error) {
      toast({
        title: 'خطأ أثناء الإضافة',
        description: error.message || 'تعذر إضافة الاستقطاع المخصص',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleUpdate = async (id) => {
    if (!editName.trim() || !editValue) {
      toast({
        title: 'تنبيه',
        description: 'لا يمكن حفظ قيم فارغة للاستقطاع المخصص',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: 'تم حفظ تعديلات بند الاستقطاع المخصص',
        variant: 'success',
      });
      ...
      await apiClient.logs.create({
        ...
        details: `تحديث البند الاستقطاعي ذو الرقم المعرف (${id}) إلى الاسم: ${editName}، القيمة: ${editValue}`
      }).catch(() => {});
    } catch (error) {
      toast({
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast({
        title: 'تم الحذف',
        description: `تم حذف الاستقطاع "${name}" بنجاح`,
        variant: 'success',
      });
      ...
      await apiClient.logs.create({
        ...
        details: `حذف بند الاستقطاع المخصص باسم (${name}) رقم المعرف: ${id}`
      }).catch(() => {});
    } catch (error) {
      toast({
        title: 'خطأ أثناء الحذف',
        description: error.message || 'تعذر حذف الاستقطاع',
        variant: 'destructive',
      });
    }
  };
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex bg-slate-100 p-1.5 rounded-xl w-fit border border-slate-200">
        ...
        >
          الاستقطاعات المؤقتة والاستثنائية
        </button>
      </div>

      {activeSubTab === 'custom' && (
        <>
          {/* 1. Fixed Legislative Deductions Card */}
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-black text-[#1B3A6B]">الاستقطاعات الثابتة والقانونية (قوانين التقاعد والخدمة العراقية)</h2>
              <p className="text-[11px] text-slate-500">تعديل نسبة الاستقطاع التقاعدي الموحد الإلزامية المطبقة على الراتب الاسمي لجميع الموظفين.</p>
            </div>
          </div>
          <button 
            onClick={fetchFixedSettings}
            className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-slate-600 transition-all"
            title="إعادة جلب الاستقطاعات الافتراضية"
          >
            <RefreshCw size={14} />
          </button>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2 bg-slate-50/30 p-4 rounded-xl border border-slate-100">
                <div className="flex justify-between items-center mb-1">
                  <label className="block text-xs font-bold text-slate-700">نسبة الاستقطاع التقاعدي الموحد</label>
                  <button
                    type="button"
                  ...
                  >
                    {retirementStatus === 'فعال' ? 'نشط/فعال' : 'متوقف مؤقتاً'}
                  </button>
                </div>
              ...
              <div className="bg-slate-50 p-4 rounded-xl flex items-center gap-3 border border-slate-100">
                ...
                <span className="text-[10px] text-slate-500 leading-relaxed">
                  يتم احتساب هذا الخصم قانونياً بشكل فوري على جميع كشوفات الرواتب للموظفين المثبتين على الملاك الدائم.
                </span>
              </div>
      ...
      {/* 2. Custom/Flexible Deductions Card */}
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-black text-[#1B3A6B]">الاستقطاعات المخصصة والمقطوعة للرواتب</h2>
              <p className="text-[11px] text-slate-500">إدارة البنود المالية الاستقطاعية الأخرى (مثلاً استقطاعات الضريبة، أقساط التأمين، اشتراكات النقابة، إلخ).</p>
            </div>
          </div>
          ...
          <div className="p-5 bg-slate-50/70 border-b border-slate-150 animate-fadeIn">
            <form onSubmit={handleAddCustom} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700">اسم الاستقطاع المخصص</label>
                <input
                  type="text"
                  value={newName}
                  ...
                  placeholder="مثال: ضريبة الدخل، اشتراك النقابة"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder={newCalcType === 'percentage' ? 'مثال: 2' : 'مثال: 5000'}
                  />
              ...
              <div className="flex gap-2">
                <button
                  type="submit"
                  className="flex-1 bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold py-2 rounded-xl flex items-center justify-center gap-1.5 shadow-sm active:scale-95 transition-all"
                >
                  <Check size={14} />
                  إضافة
                </button>
        ...
        <div className="p-6">
          {loading ? (
            <div className="py-12 flex flex-col items-center justify-center text-slate-400 gap-2">
              ...
              <span className="text-xs font-bold">جاري تحميل البنود المالية الاستقطاعية...</span>
            </div>
          ) : records.length === 0 ? (
            <div className="py-12 border-2 border-dashed border-slate-150 rounded-2xl flex flex-col items-center justify-center text-slate-400 gap-2">
              ...
              <span className="text-xs font-bold">لا توجد استقطاعات مخصصة مسجلة حالياً.</span>
              <p className="text-[10px] text-slate-400">يمكنك إضافة بنود استقطاع إضافية كالضريبة أو النقابات من الزاوية العلوية.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-right text-xs">
                <thead className="bg-slate-50 text-slate-600 font-bold border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-4 py-3">اسم بند الاستقطاع</th>
                    ...
                    return (
                      <Fragment key={r.id ?? `fixed-deduction-${idx}-${r.name || idx}`}>
                              ...
                              <div className="flex flex-col gap-1">
                                ...
                                {(() => {
                                  ...
                                  const hasConditions = 
                                    ...
                                    (rule.genders && rule.genders.length > 0) ||
                                    (rule.ethnicities && rule.ethnicities.length > 0) ||
                                    (rule.religions && rule.religions.length > 0) ||
                                    (rule.actingResponsibilities && rule.actingResponsibilities.length > 0) ||
                                    (rule.deputyLevels && rule.deputyLevels.length > 0) ||
                                    (rule.workShiftTypes && rule.workShiftTypes.length > 0) ||
                                    (rule.shiftSystems && rule.shiftSystems.length > 0);
                                  const hasBlocked = rule.blockedEmployees && rule.blockedEmployees.length > 0;
                                  if (!hasConditions && !hasBlocked) return null;
                                  return (
                                    <div className="flex flex-wrap gap-1 mt-0.5">
                                      {hasConditions && (
                                        <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[8px] bg-amber-50 text-amber-700 border border-amber-200 font-bold animate-fadeIn">
                                          شروط نشطة
                                        </span>
                                      )}
                                      {hasBlocked && (
                                        <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[8px] bg-rose-50 text-rose-700 border border-rose-200 font-bold animate-fadeIn">
                        ...
                        {expandedRuleId === r.id && currentRule && (
                          <tr className="bg-slate-50/75 select-none animate-fadeIn">
                            <td colSpan={6} className="p-6 border-b border-slate-200">
                              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-6 text-right" dir="rtl">
                                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                                  ...
                                  <span className="text-[10px] text-slate-400">إذا لم يتم تحديد أي شرط، سيُطبق الاستقطاع على جميع الموظفين تلقائياً.</span>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                  {/* 1. الدرجة والمرحلة */}
                                  <div className="space-y-2 bg-slate-50/50 p-4 rounded-xl border border-slate-100">
                                    <h5 className="font-bold text-[11px] text-slate-700">الالتحاق بالدرجة والمرحلة</h5>
                                      ...
                                      <div className="space-y-3 bg-slate-50/50 p-4 rounded-xl border border-slate-100 col-span-1 md:col-span-2 lg:col-span-3">
                                        <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-200 pb-2">
                                          <h5 className="font-bold text-xs text-rose-700 flex items-center gap-1.5">
                                            ...
                                            نظام تصفية الخاضعين بالمحددات وحجب الاستثناء
                                          </h5>
                                          <div className="flex items-center gap-1.5 mt-2 md:mt-0 flex-wrap">
                                            <span className="text-[10px] font-extrabold text-emerald-800 bg-emerald-50 px-2.5 py-1 rounded-md border border-emerald-200">
                                              الخاضعون بالمحددات: <strong>{candidateEmps.length}</strong> من <strong>{employees.length}</strong> موظف
                                            </span>
                                            ...
                                            >
                                              {showAllInRule ? 'إظهار الخاضعين بالمحددات فقط' : 'إظهار كل الموظفين'}
                                            </button>
                                            <button
                                              type="button"
                                              onClick={() => {
                                        ...
                                        <p className="text-[10px] text-slate-500 leading-relaxed">
                                          ملاحظة: تظهر القائمة أدناه الموظفين الخاضعين للاستقطاع بحسب المحددات المختارة (كالدرجة، العنوان الوظيفي، القسم). يمكن حجب أو إخضاع أي موظف استثناءً.
                                        </p>
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-sm font-black text-[#1B3A6B]">الاستقطاعات المؤقتة والاستثنائية</h2>
                  <p className="text-[11px] text-slate-500">إدارة الاستقطاعات المالية التي تقتطع لمرة واحدة في شهر وسنة محددين مع تحديد موعد الدفع وسبب الاستقطاع.</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button
            ...
            {/* Add/Edit Temporary Deduction Form */}
            {(addingTemp || editingTempRecord) && (
              <div className="p-5 bg-slate-50/70 border-b border-slate-150 animate-fadeIn">
                <form onSubmit={editingTempRecord ? handleUpdateTemporary : handleAddTemporary} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-slate-700">
                        {editingTempRecord ? 'تعديل اسم الاستقطاع المؤقت *' : 'اسم الاستقطاع المؤقت *'}
                      </label>
                      <input
                        type="text"
                        value={tempName}
                        ...
                        className="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs font-medium focus:outline-none focus:ring-1 focus:ring-[#1B3A6B]"
                        placeholder="مثال: غرامة إدارية، تسديد قسط مؤقت"
                      />
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs font-medium focus:outline-none focus:ring-1 focus:ring-[#1B3A6B]"
                        placeholder={tempCalcType === 'percentage' ? 'مثال: 5' : 'مثال: 50000'}
                      />
                    </div>

                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-slate-700">موعد الاستقطاع المالي *</label>
                      <select
                    ...
                    {tempTimingType === 'single' ? (
                      <>
                        <div className="space-y-1.5">
                          <label className="text-xs font-bold text-slate-700">السنة المالية للاستقطاع *</label>
                          <select
                        ...
                        <div className="space-y-1.5">
                          <label className="text-xs font-bold text-slate-700">الشهر المالي للاستقطاع *</label>
                          <select
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-slate-700">طريقة تحديد المستقطعين *</label>
                      <select
                  ...
                  {/* Direct Employee Selection for Temporary Deduction */}
                  {tempBeneficiaryType === 'direct' && (
                    <div className="p-4 bg-white border border-slate-200 rounded-2xl space-y-3 animate-fadeIn">
                      <label className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                        البحث وإضافة الموظفين المستقطعين مباشرة *
                      </label>
                      <div className="relative">
                        <input
                          type="text"
                      ...
                      {employeeSearch.trim() !== '' && (
                        <div className="bg-white border border-slate-200 rounded-xl max-h-48 overflow-y-auto divide-y divide-slate-100 shadow-sm">
                          {employees.filter(emp => {
                            ...
                            const title = (emp.job_title || emp.jobTitle || '').toLowerCase();
                            return (name.includes(query) || code.includes(query) || title.includes(query)) && !tempDirectEmployeeIds.map(String).includes(String(emp.id));
                          }).length === 0 ? (
                      ...
                      <div className="space-y-1.5">
                        <div className="text-[11px] font-bold text-slate-500">المستقطعون المختارون حالياً ({tempDirectEmployeeIds.length}):</div>
                        {tempDirectEmployeeIds.length === 0 ? (
                          <div className="text-[11px] text-slate-400 italic">يرجى البحث واختيار موظف واحد على الأقل ليتم تطبيق هذا الاستقطاع عليه.</div>
                        ) : (
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-700">سبب الاستقطاع المؤقت *</label>
                    <input
                      type="text"
                      value={tempReason}
                      ...
                      placeholder="اكتب تفاصيل سبب الاستقطاع والمناسبة..."
                    />
                  </div>
                  ...
                  <div className="flex justify-end gap-2 pt-2 border-t border-slate-200">
                    ...
                    >
                      {editingTempRecord ? 'حفظ التعديلات' : 'حفظ الاستقطاع المؤقت'}
                    </button>
                    <button
                      type="button"
                      onClick={() => {
            ...
            {/* Temporary Deductions List */}
            <div className="p-0">
              {unfilteredRecords.filter(r => {
                const meta = getTempMeta(r.id);
                return r.type === 'deduction' && meta && meta.isTemporary;
              }).length === 0 ? (
                <div className="text-center py-12 text-slate-400 text-xs italic">
                  لا توجد استقطاعات مؤقتة مضافة حالياً. اضغط "استقطاع مؤقت جديد" لإدراج واحد.
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-right text-xs">
                    <thead className="bg-slate-50/70 border-b border-slate-100 text-[#1B3A6B] font-bold">
                      <tr>
                        <th className="px-5 py-3">اسم الاستقطاع</th>
                        <th className="px-5 py-3">القيمة</th>
                        <th className="px-5 py-3">موعد الاستقطاع (شهر/سنة)</th>
                        <th className="px-5 py-3">السبب</th>
                        <th className="px-5 py-3">طريقة الشمول</th>
                              ...
                              <tr className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="px-5 py-3.5 font-mono">
                                  {rec.calcType === 'percentage' || rec.calc_type === 'percentage'
                                    ? `${rec.value}% من الراتب`
                                    : (typeof formatCurrency === 'function' ? formatCurrency(rec.value) : `${rec.value} د.ع`)}
                                </td>
                                  ...
                                  <td colSpan={7} className="px-6 py-4 bg-slate-50/40">
                                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-6 text-right" dir="rtl">
                                      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                                        <div className="flex items-center gap-2">
                                          <Sliders className="w-4 h-4 text-amber-600" />
                                          <h4 className="font-black text-xs text-[#1B3A6B]">شروط استحقاق وموظفي حجب الاستقطاع المؤقت: "{rec.name}"</h4>
                                        </div>
                                        <span className="text-[10px] text-slate-400">إذا لم يتم تحديد أي شرط، سيُطبق الاستقطاع على جميع الموظفين تلقائياً.</span>
                                      </div>
                                      ...
                                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        ...
                                        <div className="space-y-4 bg-slate-50/50 p-4 rounded-xl border border-slate-100 col-span-1 md:col-span-2 lg:col-span-3">
                                          <div className="flex justify-between items-center border-b border-slate-200 pb-2">
                                            <h5 className="font-bold text-xs text-slate-700">تخصيص العناوين الوظيفية وجهات العمل (الأقسام)</h5>
                                            <span className="text-[10px] text-slate-400">ابحث وحدد المسميات والأقسام المشمولة بالاستقطاع</span>
                                          </div>
                                  ...
                                  <td colSpan={7} className="px-6 py-4 bg-slate-50/50">
                                    <div className="p-4 bg-white border border-slate-200 rounded-xl space-y-3">
                                      <div className="flex justify-between items-center pb-2 border-b border-slate-200">
                                        <span className="text-xs font-bold text-[#1B3A6B]">قائمة الموظفين المستقطع منهم المباشرين ({meta.directEmployeeIds?.length || 0})</span>
                                      </div>
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 bg-slate-50 p-3 rounded-lg border border-slate-100">
                                        <div className="space-y-1">
                                          <label className="text-[10px] font-bold text-slate-600 block">البحث وإضافة موظف جديد للمستقطع منهم:</label>
                                          <div className="relative">
                                            <input
                                            type="text"
                                            placeholder="ابحث باسم الموظف، رقم الشركة، الرقم الوظيفي، العنوان أو القسم..."
                                      ...
                                      <div className="space-y-1">
                                        <label className="text-[10px] font-bold text-slate-600 block">البحث والفلترة داخل قائمة المستقطع منهم حالياً:</label>
                                        <input
                                          type="text"
      ...
      <div className="bg-amber-50/60 rounded-2xl border border-amber-200/60 p-5 flex items-start gap-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-[11px] text-amber-800 leading-relaxed">
            يتم احتساب الاستقطاعات الثابتة والمخصصة تلقائياً وتنعكس على الفور في سلم الراتب الشهري وكشوفات الرواتب لجميع الموظفين. يمكنك إعادة ترتيب الاستقطاعات المخصصة عن طريق سحب الصفوف يدوياً لتغيير أولويتها أو ترتيب ظهورها في التقارير والمسيرات.
          </p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl border border-slate-200 max-w-sm w-full p-6 animate-scaleIn shadow-2xl">
            ...
            <p className="text-xs text-slate-500 leading-relaxed mb-6">
              هل أنت متأكد من رغبتك في حذف بند الاستقطاع المخصص <strong className="text-slate-800">"{deleteConfirm.name}"</strong>؟ هذا الإجراء نهائي ولا يمكن التراجع عنه وسيتوقف استقطاعه فوراً من كافة كشوفات الرواتب.
            </p>
            <div className="flex gap-2">
              <button
